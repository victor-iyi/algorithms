"""Matrix is a special case of two dimensional array where each data element is of strictly same size. So every
matrix is also a two dimensional array but not vice versa. Matrices are very important data structures for many
mathematical and scientific calculations.

Numpy is a third-party C-optimized library which has extensive support for scientific and mathematical calculations.
NumPy has a relatively simple API with lots of functionality widely adopted in the Python scientific community.
...
Matrix Example: Consider the case of reading temperature for 1 week measured in the morning, mid-day, evening and
mid-night. We can determine the shape of the matrix by using the `.shape` attribute on the data object. In this case
